...
sudo apt install -y libqwt-qt5-6 libqwt-qt5-6 libqwt-qt5-dev g++ make libsysfs-dev automake \
unzip apt-utils libqt5webkit5-dev libpcap-dev libgps28 libgps-dev libfftw3-dev libhamlib-dev \
libasound2-dev libspeex1 libspeexdsp1 libtool libtool-bin libqt5svg5 libqt5svg5-dev \
libpulse-dev qt5-qmake libpcap-dev libspeexdsp-dev subversion libsndfile1-dev zlib1g-dev \
autoconf
...
./configure --with-pic --enable-shared --enable-drm

#Compile the faac libraries with three CPU cores
make -j3

#Install the faac libraries
sudo make install

#Link the faac library files library files
sudo ldconfig

#Change the current directory up one level to ~/src/DREAM
cd ~/src/DREAM
