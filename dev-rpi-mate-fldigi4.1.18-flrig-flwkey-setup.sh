#!/bin/sh
#install fldigi(4.1.18) w/Hamlib(4.2) flrig(1.4.0) flwkey(1.2.3) 
#N4XWE 6-19-2021
...
libusb-1.0-0 libgd-dev libhamlib-utils libsamplerate0 libsamplerate0-dev libsigc++-2.0-dev libpopt-dev \
...
libfile-stripnondeterminism-perl libfltk-cairo1 libfltk-forms1.3 libfltk-gl1.3 libfltk-images1.3 libfltk1.3 \
libfltk1.3-dev libflxmlrpc-dev libflxmlrpc1 libhamlib-dev libhamlib2 libmbedcrypto3 libmbedtls-dev libmbedtls12 \ 
...
#Download the Hamlib 4.2 source code from Sourceforge
wget -N https://sourceforge.net/projects/hamlib/files/hamlib/4.2/hamlib-4.2.tar.gz ||
  { echo 'Unable to download the HamLib source code file'; exit 1; }
  
#Extract the Hamlib source code files
tar -xvzf hamlib-4.2.tar.gz

#Make the directory containing the uncompressed Hamlib source code the current directory
cd ~/src/FLDIGI/hamlib-4.2
...
#Download the fldigi-4.1.18 source code from Sourceforge
wget -N https://sourceforge.net/projects/fldigi/files/fldigi/fldigi-4.1.18.tar.gz ||
  { echo 'Unable to download the fldigi source code file'; exit 1; }

#Extract the fldigi source code files
tar -xvzf fldigi-4.1.18.tar.gz ||
...
#Download the flrig 1.4.0 source code from Sourceforge
wget -N https://sourceforge.net/projects/fldigi/files/flrig/flrig-1.4.0.tar.gz ||
  { echo 'Unable to download the flrig source code file'; exit 1; }

#Extract the flrig source code files
tar -xvzf flrig-1.4.0.tar.gz ||
